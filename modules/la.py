...
def old_to_new_codes() -> dict:
    hdf = get_la_df(include_historical=True)
    hdf = hdf[["local-authority-code", "local-authority-type",
               "official-name", "replaced-by"]]
    hdf = hdf[~hdf["replaced-by"].isna()]
    hdf = hdf[~(hdf["local-authority-type"] == "CTY")]
    old_new_map = hdf.common.to_map("local-authority-code", "replaced-by")
    old_new_map = {x: old_new_map.get(y, y) for x, y in old_new_map.items()}
    return old_new_map
...
def get_la_with_leagues() -> pd.DataFrame:
    ...
    """
    unitaries = ['Welsh unitary authority', 'Scottish unitary authority',
                 'Unitary authority', 'Metropolitan district', 'London borough', 'City corporation']
...
def name_registry_lookup(allow_none: bool = False) -> Callable:
    ...
    banned_words = ["council", "unitary", " cc"]
...
class LAPDAccessor(object):
    ...
    def current(self) -> pd.DataFrame:
        """
        return current local authority information
        """
        return get_la_df()

    def historical(self) -> pd.DataFrame:
        """
        return all time including former authorities
        """
        return get_la_df(include_historical=True)
    ...
    def get_council_info(self,
        ...
        df = self._obj
        add_index = False
        if df.index.name == "local-authority-code":
            df = df.reset_index()
            add_index = True
        ...
        df = df.merge(adf, how=merge_type)
        if add_index:
            df = df.set_index("local-authority-code")
        return df


    add_info = get_council_info
    ...
    def code_from_gss(self,
        ...
        """
        add a local-authority-code column based on named source column
        """
        return self.create_code_column("gss",
                                       source_col,
                                       code_col_name,
                                       allow_none,
                                       set_index)
    ...
    def to_current(self,
        ...
        'weight_on' will give a mean that's been weighted by a column in
        the lookup table. 
...
class LASeriesAccessor(object):
    ...
    def update_authority_codes(self) -> pd.Series:
        """
        given a column of authority codes, update
        """
        code_lookup = old_to_new_codes()
        return self._obj.common.update_from_map(code_lookup)
